...
def event_identification(arena):        # note: You can tweak this function in case you need to give more inputs 
    ...
    '''
    event_list.append(arena[598:648,155:205])
    event_list.append(arena[468:518,460:510])
    event_list.append(arena[336:386,465:515])
    event_list.append(arena[335:385,146:196])
    event_list.append(arena[119:169, 159:209])
    event_list[0] = cv.resize(event_list[0],(50,50))
    event_list[1] = cv.resize(event_list[1],(50,50))
    event_list[2] = cv.resize(event_list[2],(50,50))
    event_list[3] = cv.resize(event_list[3],(50,50))
    event_list[4] = cv.resize(event_list[4],(50,50))
    return event_list
...
def classify_event(image):
    ...
    imge = cv.cvtColor(image, cv.COLOR_BGR2RGB)
    Tr = transforms.ToPILImage()
    imge = Tr(imge)
    #imge.show()
    convert_tensor = transforms.ToTensor()
    imge = convert_tensor(imge)
    imge = imge.unsqueeze(0)
    model = torch.load(r"C:\Users\mouli\Downloads\EYantra\trainedmodel.pth")
    outputs = model(imge)
